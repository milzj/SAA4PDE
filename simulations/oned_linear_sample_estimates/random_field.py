...
class RandomField(object):
	...
	def __init__(self, mean=1.0, var=np.exp(-1.125)):
		"""
		Implements a modification of the random field in sect. 8 in Martin, Krumscheid, and Nobile (2021).
	...
	def function_space(self, function_space):
		...
		self.g = Expression("xi1*cos(1.1*pi*x[0])+xi2*cos(1.2*pi*x[0])+xi3*sin(1.3*pi*x[0])+xi4*sin(1.4*pi*x[0])",
